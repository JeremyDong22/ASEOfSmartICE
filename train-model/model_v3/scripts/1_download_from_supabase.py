#!/usr/bin/env python3
# Version: 2.0 (Model V3)
# Download ONLY camera_35 and camera_22 images from Supabase (1080p high-quality cameras)
...
TARGET_CAMERAS = ['camera_35', 'camera_22']  # Only high-quality 1080p cameras
...
def main():
    ...
    # Query only camera_35 and camera_22 records
    print(f"📊 Querying Supabase database for {TARGET_CAMERAS}...")
    response = supabase.table('ase_snapshot')\
        .select('*')\
        .in_('camera_name', TARGET_CAMERAS)\
        .execute()
    records = response.data

    total_images = len(records)
    print(f"Found {total_images} images from {TARGET_CAMERAS} to download\n")
