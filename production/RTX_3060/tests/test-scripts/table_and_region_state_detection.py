...
PERSON_DETECTOR_MODEL = str(SCRIPT_DIR.parent.parent / "models" / "yolov8m.pt")
STAFF_CLASSIFIER_MODEL = str(SCRIPT_DIR.parent.parent / "models" / "waiter_customer_classifier.pt")
...
# Configuration file
CONFIG_FILE = str(SCRIPT_DIR / "table_region_config.json")
...
class PerformanceTracker:
    ...
    def print_summary(self, video_duration, original_fps):
        ...
        print(f"   Avg frame time: {(total_time/self.total_frames)*1000:.1f}ms")
        print(f"")
...
def init_database(db_path):
    ...
    - screenshots: Links to saved screenshot files
    """
...
def save_screenshot(frame, screenshot_dir, session_id, frame_number, prefix=""):
    ...
    return str(filepath.relative_to(screenshot_path.parent.parent))


def log_division_state_change(conn, session_id, frame_number, timestamp, state,
                              walking_waiters, service_waiters, screenshot_path):
    """Log division state change to database"""
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO division_states
        (session_id, frame_number, timestamp, state, walking_area_waiters,
         service_area_waiters, screenshot_path)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', (session_id, frame_number, timestamp, state, walking_waiters,
          service_waiters, screenshot_path))
    conn.commit()


def log_table_state_change(conn, session_id, frame_number, timestamp, table_id,
                           state, customers, waiters, screenshot_path):
    """Log table state change to database"""
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO table_states
        (session_id, frame_number, timestamp, table_id, state,
...
def process_video(video_path, person_detector, staff_classifier, config, output_dir=None, duration_limit=None):
    ...
    cursor.execute('''
        ...
        (session_id, video_file, start_time, fps, resolution, config_file)
        VALUES (?, ?, ?, ?, ?, ?)
    ...
    screenshot_dir = db_dir / "screenshots" / session_id
    ...
    try:
        while True:
            ...
            # Save screenshots and log state changes to database
            for table in changed_tables:
                screenshot_path = save_screenshot(
                    annotated_frame, screenshot_dir, session_id,
                    frame_idx, prefix=f"{table.id}_")
                log_table_state_change(
                    conn, session_id, frame_idx, current_time,
...
Examples:
  ...
  python3 table_and_region_state_detection.py --video ../test-videos/camera_35.mp4 --interactive

  # Process with existing config
  python3 table_and_region_state_detection.py --video ../test-videos/camera_35.mp4 --duration 60

  # Process full video
  python3 table_and_region_state_detection.py --video ../test-videos/camera_35.mp4
        """
    )
    parser.add_argument("--video", required=True, help="Path to input video")
    parser.add_argument("--output", default=str(SCRIPT_DIR.parent / "test-results"),
